...
public class Playermovement : MonoBehaviour
{
    ...
    // === NUEVA FUNCIÓN PARA CONTROLAR LA CAÍDA AL VACÍO ===
    public void TeletransportarA(Vector3 nuevaPosicion)
    {
        // 1. Mover al personaje al SpawnPoint asignado
        transform.position = nuevaPosicion;

        // 2. Detener por completo las fuerzas de caída acumuladas
        if (rb != null)
        {
        ...
        // 3. Resetear el contador de saltos para que pueda saltar apenas aparezca
        saltosRestantes = saltosTotalesPermitidos;

        Debug.Log("¡El jugador cayó al vacío y regresó a salvo al SpawnPoint!");
    }
}