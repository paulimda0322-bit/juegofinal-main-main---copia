...
public class PlataformaController : MonoBehaviour
{
    ...
    // === NUEVA FUNCIÓN PÚBLICA PARA MANEJAR LA REAPARICIÓN ===
    public void TeletransportarA(Vector3 nuevaPosicion)
    {
        // 1. Mover la posición del personaje al SpawnPoint asignado
        transform.position = nuevaPosicion;

        // 2. Detener por completo la velocidad y fuerzas acumuladas por la caída
        if (rb != null)
        {
        ...
        // 3. Resetear lógicamente el doble salto por seguridad al reaparecer en el aire/plataforma
        puedeDobleSalto = true;

        Debug.Log("🍄 ¡El personaje cayó al vacío y regresó a salvo!");
    }
}